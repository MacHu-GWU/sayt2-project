...
_ACQUIRE_SQL = """\
INSERT INTO locks (name, lock, lock_at) VALUES (?, ?, ?)
ON CONFLICT(name) DO UPDATE SET lock = excluded.lock, lock_at = excluded.lock_at
WHERE locks.lock IS NULL OR locks.lock_at < ?
"""

_RELEASE_SQL = "UPDATE locks SET lock = NULL, lock_at = NULL WHERE name = ?"
...
class Tracker:
    ...
    def lock_it(self, name: str, expire: int = 60) -> str:
        ...
        expired_threshold = (_utcnow() - timedelta(seconds=expire)).isoformat()

        with self._get_conn() as conn:
            cursor = self._execute(
                conn, _ACQUIRE_SQL, (name, lock_token, now_iso, expired_threshold)
            )
    ...
    def unlock_it(self, name: str) -> None:
        """Release the lock for *name*."""
        with self._get_conn() as conn:
            self._execute(conn, _RELEASE_SQL, (name,))
